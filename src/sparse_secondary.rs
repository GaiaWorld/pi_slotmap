...
impl<K: Key, V, S: hash::BuildHasher> SparseSecondaryMap<K, V, S> {
    ...
    pub fn contains_key(&self, key: K) -> bool {
        let kd = key.data();
        self.slots.get(&kd.index()).map_or(false, |slot| slot.version == kd.version())
    }
    ...
    pub fn insert(&mut self, key: K, value: V) -> Option<V> {
        ...
        if let Some(slot) = self.slots.get_mut(&kd.index()) {
        ...
        self.slots.insert(kd.index(), Slot {
            version: kd.version(),
            value,
        });
    ...
    pub fn remove(&mut self, key: K) -> Option<V> {
        ...
        if let hash_map::Entry::Occupied(entry) = self.slots.entry(kd.index()) {
        ...
        F: FnMut(K, &mut V) -> bool,
    {
        self.slots.retain(|&idx, slot| {
            let key = unsafe { key_data(idx, slot.version).into() };
    ...
    pub fn get_disjoint_mut<const N: usize>(&mut self, keys: [K; N]) -> Option<[&mut V; N]> {
        ...
        while i < N {
            ...
            match self.slots.get_mut(&kd.index()) {
        ...
        for k in &keys[0..i] {
            match self.slots.get_mut(&k.data().index()) {
                Some(Slot { version, .. }) => {
                    *version ^= 1;
                },
    ...
    pub fn entry(&mut self, key: K) -> Option<Entry<K, V>> {
        ...
        if let hash_map::Entry::Occupied(o) = self.slots.entry(kd.index()) {
        ...
        Some(match self.slots.entry(kd.index()) {
